...
//
// This is a lightly modified vendored copy of github.com/rakyll/portmidi.
// The only change from upstream is the set of cgo build directives below,
// which are made platform-aware so the project builds out of the box on:
//
//   - macOS Apple Silicon  (Homebrew prefix /opt/homebrew)
//   - macOS Intel          (Homebrew prefix /usr/local)
//   - Linux amd64 / arm64  (system libportmidi in the default search path)
//
// Non-existent -I / -L directories are ignored by the toolchain, so listing
// several candidate prefixes is harmless. You can still override everything
// via the CGO_CFLAGS / CGO_LDFLAGS environment variables for non-standard
// install locations.
package portmidi
